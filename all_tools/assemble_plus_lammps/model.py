...
from conversions import read_from_string, write_to_string
from xyz import unsaturated_xyz_to_xyz
import const
...
class NodeDescription:
    ...
    xyz: str or None = field(default=None, repr=False)
...
class LigandTemplate:
    ...
    def prepare_inputs(self) -> tuple[list[str], np.ndarray, np.ndarray or None]:
    ...
    @classmethod
    def from_yaml(cls, path: Path or str) -> 'LigandTemplate':
...
class LigandDescription:
    ...
    smiles: str or None = field(default=None)
    ...
    anchor_type: str or None = field(default=None)
    """Name of the functional group used for anchoring"""
    prompt_atoms: list[list[int]] or None = field(default=None, repr=True)
    ...
    metadata: dict[str, int or float or str] = field(default_factory=dict)
...
class MOFRecord:
    ...
    structure: str or None = field(default=None, repr=False)
    ...
    # Properties
    gas_storage: dict[str, float or tuple[float, float]] = field(default_factory=dict, repr=False)  # TODO (wardlt): Allow only one type of value
    ...
    @classmethod
    def from_file(cls, path: Path or str, read_kwargs: dict or None = None, **kwargs) -> 'MOFRecord':
...
if __name__ == "__main__":
    cif_path = '/home/x-sappana/MOFScientist/output_dir/cifs/hMOF-17.cif'
    mof_record = MOFRecord.from_file(cif_path)
    print(mof_record.__dict__)
